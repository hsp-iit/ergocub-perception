...
if __name__ == "__main__":
    ...
    create_engine(  # p,
        'ISBFSAR/modules/hpe/weights/onnxs/yolo.onnx',
        'ISBFSAR/modules/hpe/weights/engines/docker/yolo.engine',
        i)
    ...
    create_engine(  # p,
        'ISBFSAR/modules/hpe/weights/onnxs/image_transformation{}.onnx'.format(BATCH_SIZE),
        'ISBFSAR/modules/hpe/weights/engines/docker/image_transformation{}.engine'.format(BATCH_SIZE),
        i)

    # BackBone
    i = {"images": np.ones(shape=(BATCH_SIZE, 256, 256, 3), dtype=np.float32)}
    create_engine(  # p,
        'ISBFSAR/modules/hpe/weights/onnxs/bbone{}.onnx'.format(BATCH_SIZE),
        'ISBFSAR/modules/hpe/weights/engines/docker/bbone{}.engine'.format(BATCH_SIZE),
        i)
    # Heads
    i = {"input": np.ones(shape=(81920*BATCH_SIZE,), dtype=np.float32)}
    create_engine(  # p,
        'ISBFSAR/modules/hpe/weights/onnxs/heads{}.onnx'.format(BATCH_SIZE),
        'ISBFSAR/modules/hpe/weights/engines/docker/heads{}.engine'.format(BATCH_SIZE),
        i)
